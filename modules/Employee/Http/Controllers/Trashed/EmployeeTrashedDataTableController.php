...
class EmployeeTrashedDataTableController extends Controller
{
    public function __invoke(Request $request)
    {
        if ($request->ajax()) {
                    $model = Employee::onlyTrashed();
                    ...
                    ->addColumn('action', function($row){
                        $buttons = [
                            _dt_btn_factory([
                                ...
                                'actionType'  => 'alert',
                                'actionMethod'  => 'POST',
                                'url'         => route('employee.trashed.restore', [$row->id]),
                                ...
                                'permissions' => auth()->user()->can('employee.actions.restore'),
                                'conditions'    => true,
                                'tooltip' => [
                                    'disabled' => false,
                                    'placement' => 'top',
                            ...
                            _dt_btn_factory([
                                ...
                                'actionType'  => 'alert',
                                'actionMethod'  => 'DELETE',
                                'url'         => route('employee.trashed.destroy', [$row->id]),
                                'title'       => Lang::get("core::global.datatable.actions.delete"),
                                'icon'        => 'fas fa-trash',
                                'classes'     => 'btn btn-icon btn-light-danger btn-sm',
                                'permissions' => auth()->user()->can('employee.actions.hard_delete'),
                                'conditions'    => true,
                                'tooltip' => [
                                    'disabled' => false,
                                    'placement' => 'top',
